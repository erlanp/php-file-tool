...
<form id="mainform" name="mainform" target="iframe_file" method="post" enctype="multipart/form-data">
	...
	<div>
	<?php echo form_label('处理函数:'); ?>
	<?php echo form_input(array(
              'name'        => 'fn_copy',
              'id'          => 'fn_copy',
              'value'       => $input->get_post('fn_copy'),
              'size'        => '65',
            )); ?>
	</div>
	...
	<textarea rows="2" cols="120" name="suffix"><?php 
		echo $input->get_post('suffix') ?: '[".php", ".html", ".js", ".css", ".java", ".bat", ".xml", ".yml", ".properties"]'; 
	?></textarea>
	</div>
	<div>
	<?php echo form_label('禁用文件后缀:'); ?>
	</div>
	<div>
	<textarea rows="2" cols="120" name="black_suffix"><?php 
		echo $input->get_post('black_suffix') ?: '["tests.java", "test.java"]'; 
	?></textarea>
	</div>
	...
	<div>
	<textarea rows="2" cols="120" name="allow_to_copy"><?php 
		echo $input->get_post('allow_to_copy') ?: ''; 
	?></textarea>
	</div>
	
	<div>
	<?php echo form_label('禁止复制文件:'); ?>
	</div>
	<div>
	<textarea rows="2" cols="120" name="prohibited_to_copy"><?php 
		echo $input->get_post('prohibited_to_copy') ?: '["config.php"]'; 
	?></textarea>
	</div>
